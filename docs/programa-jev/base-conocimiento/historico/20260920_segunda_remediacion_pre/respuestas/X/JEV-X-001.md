...
# JEV-X-001 — Síntesis Transversal, Arquitectura y Gobernanza Integral

## 1. Respuesta directa y decisión que permite tomar
La explicación unificada de Jev 1.13.0 exige distinguir estrictamente cuatro capas: (1) Producto: servicio gestionado de inferencia semántica ligera de Sistema 1 alojado por TypeSafe AI, accesible únicamente vía `POST /v1/systemone`; (2) Arquitectura: modelo de lenguaje compacto optimizado para latencia y coste ($0.042/Mtok de entrada, salida gratuita) sin memoria conversacional; (3) Contrato de salida: primitivas tipadas estrictas (`Choice` devuelve `choice`, `probabilities`, `confidence`; `Score` devuelve `score`, `legend`, `probabilities`, `confidence`; `Noul` devuelve exclusivamente `noul` en [0, 1] sin confianza nativa); y (4) Política de acción: lógica cliente determinista en CPU que evalúa umbrales de confianza, abstención y enrutamiento a reglas locales o revisión humana.
...
Confundir el contrato de salida de la API con la política de decisión del cliente es el error arquitectónico más frecuente.
...
logger = logging.getLogger("JEV-X-001")
...
- **Registro de consulta NotebookLM:** Cuaderno Pilar X (`7051ceed-3234-4060-96c5-93e0267212c1`), consulta registrada el 2026-09-20, archivo de evidencia: `base-conocimiento/remediacion-integral/consultas/JEV-X-001.json`.
