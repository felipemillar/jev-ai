...
Clase canónica de `typesafe-sdk==0.7.0` (módulo interno `typesafe_sdk._core.retry.RetryPolicy`, importación pública `from typesafe_sdk import RetryPolicy`) para gobernar reintentos automáticos ante errores de transporte o códigos HTTP 429 transitorios. Sus parámetros oficiales son: `max_retries` (int, por defecto 2), `backoff_initial` (float, por defecto 0.5s o calibrable a 1.0s), `backoff_max` (float, por defecto 5.0s o calibrable a 30.0s), `backoff_jitter` (float, por defecto 0.25; valor numérico no booleano) y `timeout` (float | None, por defecto 30.0s). El parámetro obsoleto `backoff_factor` ha sido eliminado del SDK 0.7.0 en favor de la decorrelación exponencial con jitter.
